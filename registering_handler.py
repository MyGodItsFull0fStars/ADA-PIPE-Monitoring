...
class RegisteringHandler:
    ...
    def __init__(self) -> None:
        ...
        self.master_node_register_url: str = self._network_handler.get_master_node_url('register')

    def register(self):
        payload = self._network_handler.get_registering_payload()
        response = requests.post(url=self.master_node_register_url, json=payload)
    ...
    def unregister(self):
        ...
        response = requests.delete(url=self.master_node_register_url, json=payload)
        print(response.status_code, response.text)
